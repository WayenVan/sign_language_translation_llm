...
class SignCL(nn.Module):
    ...
    def forward(self, inputs_embeds, margin=20):
        ...
        for i in range(1, seq_len - 2):
            anchor = inputs_embeds[:, i, :].unsqueeze(
                1
            )  # Anchor sample of shape (batch_size, 1, embed_dim)

            # Positive samples selection
            pos_indices = [
                idx for idx in range(max(0, i - 1), min(seq_len, i + 2)) if idx != i
            ]
            selected_pos_indices = random.sample(
                pos_indices, k=min(len(pos_indices), self.pos_samples)
            )
            positives = inputs_embeds[
            ...
            neg_indices = [
                idx
                for idx in range(2, seq_len - 2)
                if idx < i - margin or idx > i + margin
            ]
            selected_neg_indices = random.sample(
                neg_indices, k=min(len(neg_indices), self.neg_samples)
            )
            negatives = inputs_embeds[
                :, selected_neg_indices, :
            ]  # Negative samples of shape (batch_size, neg_samples, embed_dim)

            # Calculate distances
            pos_dist = torch.sum(
                torch.abs(anchor - positives), dim=-1
            )  # Distance to positive samples
            neg_dist = torch.sum(
                torch.abs(anchor - negatives), dim=-1
